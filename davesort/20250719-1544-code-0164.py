...
def download_system_debs():
    ...
    if not deb_files or (len(deb_files) == 1 and deb_files[0] == ''):
        ...
        deb_files = [f for f = f in deb_files if f] # Filter out empty strings
...
def clone_git_repos():
    ...
    for repo_url, repo_name in repos_to_clone:
        ...
        if os.path.exists(target_path) and os.listdir(target_path): # Check if dir exists AND is not empty
