...
<head>
    ...
    <style>body{background:#191a21;color:#eee;font-family:'Segoe UI',Arial,sans-serif;}
    ...
    input{font-size:16px;border-radius:6px;padding:4px 7px;}pre{background:#21232b;color:#def;padding:8px;border-radius:8px;}
    .file-list{font-size:14px;margin-bottom:10px;}.stats-table td{padding:3px 9px;}.stats-table th{color:#ffd700;text-align:left;}
    </style>
    <script>
    async function refresh(){
        let s = await fetch("/api/status"); let d = await s.json();
        document.getElementById("stats").innerHTML = `<table class=stats-table>
        <tr><th>Host</th><td>${d.hostname} (${d.type})</td></tr>
        ...
        <tr><th>Plugins</th><td>${(d.plugins||[]).join(", ")||"None"}</td></tr>
...
<div style="max-width:900px;margin:36px auto;">
    ...
    <button onclick="fetch('/api/reboot',{method:'POST'});setTimeout(refresh,3000);">Reboot</button>
    <button onclick="fetch('/api/update',{method:'POST'});setTimeout(refresh,3000);">Update</button>
...
if __name__ == "__main__":
    print("[JEMAI] Cluster AGI agent is LIVE at http://localhost:8181")
    if IS_WINDOWS:
