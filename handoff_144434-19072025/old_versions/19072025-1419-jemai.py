...
<head>
  ...
  function updateStatus(d) {
      document.getElementById("rb_stats").innerHTML = `
        ...
        <b>Plugins:</b> ${(d.plugins||[]).map(x=>'<span class="chip" onclick="triggerPlugin(\\''+x+'\\')">'+x+'</span>').join('')||"None"}<br>
...
def rollback_ui():
    ...
    return render_template_string("""
    <html><body style='background:#262e3b;color:#eee;font-family:monospace;padding:30px;'>
    <h2>JEMAI Rollback Utility</h2>
    <ul>
    {% for f in files %}
      <li>
        <a href='/api/preview/{{f}}' target='_blank'>{{f}}</a>
        <a href='/api/compare/{{f}}' style='color:#f95;'>[Diff]</a>
        <a href='/api/activate/{{f}}' style='color:#0f8;'>[Roll back]</a>
      </li>
    {% endfor %}
    </ul>
    <a href='/'>Back to OS</a>
...
@app.route("/api/preview/<fname>")
def preview_code(fname):
    path = os.path.join(VERSIONS_DIR, fname)
    if not os.path.exists(path): return "File not found"
    with open(path, encoding="utf-8") as f: code = f.read()
    return f"<pre style='background:#181c27;color:#bfe;font-size:1.03em;padding:14px;'>{code}</pre>"

@app.route("/api/compare/<fname>")
def compare_code(fname):
    cur = open(__file__, encoding="utf-8").read().splitlines()
    old = open(os.path.join(VERSIONS_DIR, fname), encoding="utf-8").read().splitlines()
    diff = difflib.unified_diff(old, cur, fromfile=fname, tofile="Current")
    return "<pre style='background:#181c27;color:#bfe;font-size:1.03em;padding:14px;'>" + "\n".join(diff) + "</pre>"

@app.route("/api/activate/<fname>")
def activate_code(fname):
    src = os.path.join(VERSIONS_DIR, fname)
    if not os.path.exists(src): return "File not found"
    shutil.copy2(src, __file__)
    save_wiki(f"Rollback: Activated version {fname}")
    return redirect("/")

@app.route("/wiki")
def wiki_ui():
    return render_template_string("""
    <html><body style='background:#232c34;color:#e9e4dc;font-family:sans-serif;padding:55px;'>
    <h2>JEMAI WIKI</h2>
    <pre style="background:#2a3441;color:#efe;padding:16px;">{{wiki}}</pre>
    <a href='/'>Back to OS</a>
    </body></html>
    """, wiki=get_wiki())
