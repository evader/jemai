...
os.makedirs(os.path.join(JEMAI_HUB, "uploads"), exist_ok=True)
os.makedirs(os.path.join(JEMAI_HUB, "chat_data"), exist_ok=True)
os.makedirs(os.path.join(JEMAI_HUB, "audio"), exist_ok=True)
...
# === CLIPBOARD/OVERLAY BAKED-IN (MIN) ===
...
::-webkit-scrollbar{width:7px;background:#2e3353;}::-webkit-scrollbar-thumb{background:#5356a2;border-radius:4px;}
</style>
<script>
...
let rec=new webkitSpeechRecognition();rec.continuous=false;rec.interimResults=false;
rec.lang='en-US';
...
def api_chat():
    ...
    if q.strip().lower().startswith("search "):
        ...
        if plugin in PLUGIN_FUNCS:
            try:resp=f"[PLUGIN {plugin}]: {PLUGIN_FUNCS[plugin]()}"
            except Exception as e:resp=f"[PLUGIN {plugin} ERR]: {e}"
        else:resp=f"No such plugin '{plugin}'."
    elif q.strip().lower() in PLUGIN_FUNCS:
        try:resp=f"[PLUGIN {q.strip()}]: {PLUGIN_FUNCS[q.strip()]()}"
        except Exception as e:resp=f"[PLUGIN {q.strip()} ERR]: {e}"
    else:
