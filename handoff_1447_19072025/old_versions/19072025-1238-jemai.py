import os, sys, time, datetime, threading, platform, json, psutil, socket, sqlite3, base64, random, shutil
...
<head>
  ...
  .glass {
    background: rgba(38, 43, 57, 0.65);
    border-radius: 24px;
    box-shadow: 0 7px 48px #3ddad65b;
  ...
  .jemai-logo {
    ...
    color: #3ddad6;
    text-shadow:0 3px 24px #50e7c0c7, 0 2px 1px #155052;
    margin-bottom: 2px;
    font-family: 'Segoe UI',Arial,sans-serif;
  }
  .slogan {
  ...
  .mic-btn {
    ...
    background:linear-gradient(100deg,#65edcf 0,#3ddad6 90%);
    color:#112e29;
    border:none;border-radius:54px;width:72px;height:72px;
    box-shadow:0 4px 26px #3ddad633;
    font-size:2.1em;transition:box-shadow .16s,background .16s;
    cursor:pointer;margin:8px 22px 8px 0;
    outline: none;
    border: 3px solid #29bdc3;
    animation: pulse 1.5s infinite;
  }
  .mic-btn:active,.mic-btn:focus {box-shadow:0 0 12px #9af8e8;}
  @keyframes pulse{
    0%{box-shadow:0 0 12px #3ddad633;}
    60%{box-shadow:0 0 44px #3ddad688;}
    100%{box-shadow:0 0 12px #3ddad633;}
  }
  .chat-wrap {
    max-width: 900px;
    margin: 0 auto;
  ...
  .chat-box {
    ...
    padding:24px;
    margin-bottom: 10px;
    box-shadow:0 3px 22px #24576b55;
  }
  .chat-bubble {
    margin: 0 0 16px 0;
    padding: 13px 19px;
  ...
  .chat-bubble.user {
    background:#fff2a8;color:#463a13;margin-left:auto;
    border-radius:16px 16px 16px 8px;
  }
  .chat-timestamp {font-size:0.87em; color:#58dacb; margin-top:2px; margin-bottom:10px;}
  .rightbar {
    flex:1 1 260px;
    background:rgba(32,44,48,0.52);
    border-radius:18px;
    min-height:120px;
    padding:20px 12px 10px 18px;
    color:#6cebd2;
    font-size:1.06em;
    box-shadow:0 2px 18px #206f8760;
    margin-bottom: 10px;
  }
  .inputbar {
    display:flex; gap:14px; margin-top:13px; margin-bottom:20px;
  }
  .inputbar input, .inputbar textarea {
    flex:1 1 70px; border-radius:9px;
    border:none; font-size:1.18em; padding:10px 13px;
    background: #eafffb; color: #19423f;
    margin-right:7px;
    outline: none;
  }
  .inputbar button {
    background: linear-gradient(100deg,#65edcf 0,#3ddad6 90%);
    color:#112e29; border:none; border-radius:9px;
    font-size:1.15em; padding:10px 26px; cursor:pointer;
    box-shadow:0 3px 16px #3ddad644;
    transition:box-shadow 0.18s,background 0.14s;
  }
  .inputbar button:hover {background:#e2fff8;}
  .filebox {
    margin:8px 0 6px 0;font-size:0.98em;
  }
  ...
  .footer {
    margin-top:18px; color:#6edac8; font-size:1em; text-align:center; opacity:0.83;
    border-top:1px solid #235a42; padding-top:10px;
  }
  @media (max-width: 690px) {
    .glass, .chat-wrap {max-width:98vw;}
    .chat-wrap {flex-direction:column; gap:0;}
    .rightbar {margin-left:0;}
    .chat-box {min-height:210px;}
  }
  </style>
  <script>
  let chatLog = [];
  ...
  function updateStatus(d) {
      document.getElementById("rb_stats").innerHTML = `
        <b>System</b><br>
        Host: <b>${d.hostname}</b> <span class=chip>${d.type}</span><br>
        CPU: ${d.cpu}%<br>
        RAM: ${d.ram}%<br>
        Disk: ${d.disk}%<br>
      ...
      `;
      document.getElementById("rb_vers").innerHTML = `
        <b>Versions</b><br>${(d.versions||[]).map(x=>'<span class="chip">'+x+'</span>').join('')||"None"}
      `;
  }
  ...
  function loadHistory() {
    fetch('/api/history').then(r=>r.json()).then(hist=>{
      let div = document.getElementById('rb_mem');
      if(!div) return;
      div.innerHTML = '<b>Recent Memory</b><br>';
      hist.forEach(h=>{
        div.innerHTML += `<div class='chip' title='${h.text.slice(0,99)}'>${h.title||h.source}</div>`;
      });
    });
  }
  function loadFiles() {
    fetch('/api/files').then(r=>r.json()).then(list=>{
      let div = document.getElementById('rb_files');
      if(!div) return;
      div.innerHTML = '<b>JEMAI Hub Files</b><br>';
      list.forEach(f=>{
        div.innerHTML += `<div class='chip'>${f}</div>`;
      });
    });
  }
  function askJemai() {
    ...
    .then(j=>{
      ...
      if(j.resp_voice_url){
        let a = new Audio(j.resp_voice_url); a.play();
      }
    });
  }
  ...
  function startMic() {
    ...
    rec.onerror = function(e){alert("Voice error: "+e.error);}
    rec.start();
  }
  window.onload = ()=>{
    refreshRight(); loadHistory(); loadFiles();
  }
  </script>
</head>
<body>
  <div class="glass">
    <div class="jemai-logo">JEMAI AGI OS</div>
    <div class="slogan">Your voice-first AGI workspace.<br>Chat, code, search, control &mdash; all in one OS.</div>
    ...
    <div class="footer">
      <span>JEMAI AGI OS &copy; {{year}} | <a href="#" onclick="location.reload()">Refresh</a> | <span class="settings-link" onclick="alert('Coming soon: settings and device manager')">About/Settings</span></span>
    </div>
  </div>
</body>
</html>
...
def api_history():
    return jsonify(memory_history(20))
...
def api_chat():
    ...
    q = data.get("q", "")[:1000]
    # Basic response: echo, ollama, or "memory search"
    resp = ""
    ...
    # Voice synth (baked: Windows=EdgeTTS, Linux=pyttsx3, fallback none)
    voice_url = None
    try:
...
def serve_hubfile(filename):
    return send_from_directory(JEMAI_HUB, filename)
