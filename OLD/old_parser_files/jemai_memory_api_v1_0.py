...
def search_db(query: str, limit: int = 5):
    ...
    try:
        ...
        rows = cur.fetchall()
        return [{"title": r[0], "text": r[1], "source": r[2]} for r in rows]
    finally:
        conn.close()
...
def add_entry(title: str, text: str, source: str = "manual"):
    ...
    cur = conn.cursor()
    try:
        cur.execute("INSERT INTO memory (title, text, source) VALUES (?, ?, ?)", (title, text, source))
        conn.commit()
...
if __name__ == "__main__":
    init_db()
    import uvicorn
    uvicorn.run("jemai_memory_api_v1_0:app", host="0.0.0.0", port=8089, reload=True)
